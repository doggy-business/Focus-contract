// src/App.jsx
// CONTR-ACT. — Supabase-backed version
// Changes from previous version:
//   • Supabase auth (sign up / sign in / sign out)
//   • Profile loaded from Supabase on every sign-in
//   • Sessions read from and written to Supabase (not localStorage)
//   • Free plan gate counts completed sessions from Supabase
//   • Onboarding shown before auth; completion synced to profile on sign-in
//   • localStorage only used for: saved tasks, planned sessions (no user data)

import { useState, useEffect, useRef, useCallback } from "react";
import { supabase } from "./supabaseClient.js";
import { fetchProfile, createProfile, markOnboardingComplete } from "./profileService.js";
import { fetchSessions, fetchCompletedCount, insertSession, updateSession } from "./sessionService.js";

// ─── LOCAL-ONLY KEYS (not session data) ───────────────────────────────────────
...
// ─── STATS (computed from sessions array) ─────────────────────────────────────
...
.auth-toggle button{background:none;border:none;color:var(--accent);cursor:pointer;font-size:13px;font-family:'DM Sans',sans-serif;text-decoration:underline;text-underline-offset:3px;padding:0 2px}
...
.auth-divider-text{font-size:11px;color:var(--text3);font-family:'Syne',sans-serif;font-weight:700;letter-spacing:0.06em}
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#0D0D0F"}}>
      <div style={{textAlign:"center"}}>
        <div style={{fontFamily:"Syne,sans-serif",fontSize:28,fontWeight:800,color:"#fff",letterSpacing:"-0.01em",marginBottom:16}}>
          <span style={{color:"#fff"}}>CONTR-</span><span style={{color:"#E8FF47"}}>ACT.</span>
        </div>
  ...
  return(
    <div className="onboard">
      ...
      <div style={{width:"100%",maxWidth:300,display:"flex",flexDirection:"column",gap:10}}>
        {!isLast
          ?<>
            <button className="btn btn-primary" onClick={()=>setStep(p=>p+1)}>Continue</button>
            {step>0&&<button className="btn btn-ghost" style={{width:"auto",alignSelf:"center",fontSize:13}} onClick={()=>setStep(p=>p-1)}>Back</button>}
          </>
          :<>
            <div style={{fontFamily:"Syne",fontSize:12,fontWeight:700,color:"var(--text3)",textAlign:"center",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:4}}>Create your account to continue</div>
            <button className="btn btn-primary" onClick={onDone}>Create Account / Sign In</button>
            {step>0&&<button className="btn btn-ghost" style={{width:"auto",alignSelf:"center",fontSize:13}} onClick={()=>setStep(p=>p-1)}>Back</button>}
          </>
        }
      </div>
  ...
  const handleSubmit=async()=>{
    ...
    try{
      if(mode==="signup"){
        ...
        if(err)throw err;
        // Profile is created by the DB trigger. Wait a beat then signal success.
        // data.user may be null if email confirmation is required — handle both.
        if(data.user){
          onAuth(data.user);
        } else {
  ...
  return(
    <div className="auth-screen">
      ...
      <div className="auth-form">
        ...
        <input className="input" type="email" placeholder="Email address" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={onKey} autoComplete="email"/>
        <input className="input" type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={onKey} autoComplete={mode==="signup"?"new-password":"current-password"}/>
        <button className="btn btn-primary" onClick={handleSubmit} disabled={loading||!email.trim()||!password.trim()}>
          {loading?<><div className="spinner"/>{mode==="signup"?"Creating…":"Signing in…"}</>:mode==="signup"?"Create Account":"Sign In"}
        </button>
...
function PaywallScreen({onDismiss}){
  const handleUpgrade=()=>{
    ...
    // 1. POST to your backend: /api/create-checkout-session (pass userId in body)
    // 2. Backend creates a Stripe session and returns { url }
    // 3. Redirect: window.location.href = checkoutUrl
    // 4. On Stripe success webhook: UPDATE profiles SET plan = 'pro' WHERE id = userId
    // 5. Re-fetch profile in app to lift the gate
    //
    // Example:
    ...
    alert("Stripe coming soon. TODO: replace this with Stripe Checkout.");
  };
    ...
    <div className="paywall-screen">
      ...
      <div className="paywall-price-card">
        ...
        <ul className="paywall-features">
          {["Unlimited contracts","Full progress tracking","Full insights & patterns","Keep building streaks"].map(f=>(
      ...
      <div className="paywall-ctas">
        <button className="btn btn-primary" style={{fontSize:17,padding:"18px"}} onClick={handleUpgrade}>Unlock Pro</button>
...
function Step4_Mode({onNext,onBack,prefill}){
  ...
  return(
    <div className="screen">
      ...
      <div className={`mode-card ${mode==="companion"?"selected":""}`} onClick={()=>setMode("companion")}><div className="mode-icon">👥</div><div className="mode-title">Companion Mode</div><div className="mode-desc">If you try to leave, you are asked why. Task-related or quick interruption is allowed. Distraction fails the session.</div><div className="mode-badge">Best for: flexible work</div></div>
...
function FocusSession({session,onComplete,onFail}){
  ...
  const timeLeftRef=useRef(totalDuration),elapsedRef=useRef(0),hiddenAtRef=useRef(null);
  const failedRef=useRef(false),notesRef=useRef(""),timerRef=useRef(null);
  const final5Fired=useRef(false),milestoneFired=useRef({half:false,eighty:false});
...
// ─── FAILURE SCREEN ───────────────────────────────────────────────────────────
// userId passed in so we can write to Supabase
function FailureScreen({session,userId,onRestart,onDashboard,onSessionSaved}){
  ...
  const handleSubmit=async()=>{
    ...
    await insertSession(userId,{
      ...
      mode:                   session.mode,
      impact_statement:       session.impact||"",
      failure_reason:         reason,
      failure_reason_text:    reason==="other"?otherText:"",
      notes:                  session.notes||"",
      output_text:            null,
      output_category:        null,
    });
    setSaving(false);
    onSessionSaved?.(); // refresh sessions list in parent
    setSubmitted(true);
  };
...
function CompletionScreen({session,userId,stats,onDone,onSessionSaved}){
  ...
  const handleSubmit=async()=>{
    if(!output.trim()||savedRef.current)return;
    ...
    setSaving(false);
    onSessionSaved?.(); // refresh sessions + completedCount
    setStep("success");
  };
...
function PlanSession({onSave,onCancel}){
  ...
  const handleSave=()=>{if(!task||!impact.trim()||impact.trim().length<10)return;const plans=load(SK.PLANNED,[]);plans.push({id:genId(),task_name:task,timer_duration_seconds:duration,impact:impact.trim(),date,time,mode,created_at:new Date().toISOString()});save(SK.PLANNED,plans);onSave();};
  return(
    <div className="screen">
  ...
  return(
    <div className="screen">
      <div className="flex-between mb-20">
        <div>
          <h1 style={{fontFamily:"Syne",fontSize:28,fontWeight:800,letterSpacing:"-0.01em"}}><span style={{color:"var(--text)"}}>CONTR-</span><span style={{color:"var(--accent)"}}>ACT.</span></h1>
      ...
      {atLimit&&(
        <div style={{background:"rgba(232,255,71,0.06)",border:"1px solid rgba(232,255,71,0.3)",borderRadius:"var(--radius-sm)",padding:"14px 16px",marginBottom:16}}>
          <p style={{fontSize:13,color:"var(--text2)",lineHeight:1.6,marginBottom:10}}>You've completed your <strong style={{color:"var(--accent)"}}>2 free contracts</strong>. Upgrade to keep going.</p>
          <button className="btn btn-primary btn-sm" style={{width:"auto"}} onClick={onShowPaywall}>Unlock Pro — £5/month</button>
        </div>
      )}
      ...
      {stats.total>0&&(
        <div className="card mb-20" style={{display:"flex",alignItems:"center",gap:20}}>
          <div className="pct-circle-wrap">
            <svg width={80} height={80}><circle cx={40} cy={40} r={32} fill="none" stroke="var(--bg3)" strokeWidth={6}/><circle cx={40} cy={40} r={32} fill="none" stroke={completionPct>=70?"var(--success)":completionPct>=40?"var(--accent)":"var(--danger)"} strokeWidth={6} strokeLinecap="round" strokeDasharray={2*Math.PI*32} strokeDashoffset={2*Math.PI*32*(1-completionPct/100)} transform="rotate(-90 40 40)"/></svg>
            <div style={{position:"absolute",textAlign:"center"}}><div style={{fontFamily:"Syne",fontWeight:800,fontSize:18}}>{completionPct}%</div></div>
          </div>
          <div><div style={{fontFamily:"Syne",fontWeight:700,fontSize:15,marginBottom:4}}>Completion Rate</div><div className="text-sm">{completionPct>=70?"You are on a strong run. Keep it up.":completionPct>=40?"Building consistency. Try shorter sessions.":"Every session teaches you something."}</div></div>
        </div>
      )}

      {planned.length>0&&(
        <><div ref={plannedRef} className="label mb-12" style={{color:highlightPlanned?"var(--accent)":"var(--text2)",transition:"color 0.4s"}}>PLANNED SESSIONS {highlightPlanned&&<span style={{fontSize:10,background:"var(--accent)",color:"#000",borderRadius:20,padding:"1px 8px",marginLeft:6,fontFamily:"Syne",fontWeight:700}}>JUST ADDED</span>}</div>
        {planned.map((p,idx)=>(<div key={p.id} className={`plan-card ${highlightPlanned&&idx===planned.length-1?"plan-card-highlight":""}`}><div className="plan-time">{p.date} at {p.time}</div><div className="flex-between"><div><div className="plan-task">{p.task_name}</div><div className="plan-meta">{formatTime(p.timer_duration_seconds)} · {p.mode==="lockin"?"Lock-In":p.mode==="companion"?"Companion":"Phone-Down"}</div><div className="plan-impact">{p.impact.slice(0,60)}{p.impact.length>60?"...":""}</div></div><div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end",marginLeft:12}}><button className="btn btn-primary btn-sm" style={{width:"auto",whiteSpace:"nowrap"}} onClick={()=>onStartPlanned(p)}>Start</button><button className="btn btn-ghost btn-sm" style={{width:"auto",padding:"4px 10px",fontSize:11}} onClick={()=>removePlan(p.id)}>Remove</button></div></div></div>))}
      ...
      <div style={{marginTop:24,paddingTop:16,borderTop:"1px solid var(--border)"}}>
        <button onClick={onSignOut} style={{background:"none",border:"none",color:"var(--text3)",fontSize:12,cursor:"pointer",fontFamily:"DM Sans"}}>Sign out</button>
      </div>
    </div>
  );
}
...
function HistoryScreen({sessions}){
  ...
  return(
    <div className="screen">
      ...
      :filtered.map(s=>(<div key={s.id}><div className="history-item" onClick={()=>setExpanded(expanded===s.id?null:s.id)}><div className="hi-top"><div className="hi-task">{s.task_name}</div><div className={`badge ${s.status==="completed"?"badge-success":"badge-fail"}`}>{s.status==="completed"?"Done":"Failed"}</div></div><div className="hi-meta">{formatDate(s.created_at)} — {formatTime(s.time_elapsed_seconds)} focused — {s.mode} mode{s.failure_reason?` — ${FAILURE_REASONS.find(r=>r.id===s.failure_reason)?.label}`:""}</div></div>{expanded===s.id&&(<div className="card" style={{marginTop:-6,marginBottom:10,borderTopLeftRadius:0,borderTopRightRadius:0,borderTop:"none"}}><div className="text-xs mb-8">AT STAKE:</div><div className="impact-quote mb-12">{s.impact_statement}</div>{s.output_text&&<><div className="text-xs mb-4">OUTPUT:</div>{s.output_category&&<span className="output-cat mb-8">{s.output_category}</span>}<p style={{fontSize:13,color:"var(--text2)",marginTop:4}}>{s.output_text}</p></>}{s.notes&&<><div className="text-xs mt-12 mb-4">SESSION NOTES:</div><p style={{fontSize:12,color:"var(--text3)"}}>{s.notes}</p></>}</div>)}</div>))}
    </div>
  );
}
...
function InsightsScreen({sessions}){
  ...
  return(
    <div className="screen">
      ...
      {topDuration&&topDuration.total>=2&&(<div className="insight-callout mb-20"><p><strong>Best session length:</strong> Your {topDuration.mins}-minute sessions have a <strong>{topDuration.pct}% completion rate</strong>. Stick with what works.</p></div>)}
      ...
      {Object.values(stats.catTally).some(v=>v>0)&&(<><div className="label mb-12">OUTPUT CATEGORIES</div><div className="card mb-20"><div className="bar-chart">{OUTPUT_CATS.map(c=>(<div key={c} className="bar-row"><div className="bar-label">{c}</div><div className="bar-track"><div className="bar-fill" style={{width:`${(stats.catTally[c]/maxCat)*100}%`,background:"var(--accent)"}}/></div><div className="bar-count">{stats.catTally[c]}</div></div>))}</div></div></>)}
...
function ProgressScreen({sessions}){
  ...
  function buildBars(p){const res=[];if(p==="week"){for(let i=6;i>=0;i--){const d=new Date(now);d.setDate(now.getDate()-i);const key=getDayKey(d);const label=d.toLocaleDateString("en-US",{weekday:"short"}).slice(0,2);const ds=sessions.filter(s=>getDayKey(new Date(s.created_at))===key);res.push({label,completed:ds.filter(s=>s.status==="completed").length,failed:ds.filter(s=>s.status==="failed").length,total:ds.length});}}else if(p==="month"){for(let i=3;i>=0;i--){const wEnd=new Date(now);wEnd.setDate(now.getDate()-i*7);const wStart=new Date(wEnd);wStart.setDate(wEnd.getDate()-6);const ws=sessions.filter(s=>{const d=new Date(s.created_at);return d>=wStart&&d<=wEnd;});res.push({label:`W${4-i}`,completed:ws.filter(s=>s.status==="completed").length,failed:ws.filter(s=>s.status==="failed").length,total:ws.length});}}else{const mn=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];for(let i=11;i>=0;i--){const d=new Date(now);d.setMonth(now.getMonth()-i);const y=d.getFullYear(),m=d.getMonth();const ms=sessions.filter(s=>{const sd=new Date(s.created_at);return sd.getFullYear()===y&&sd.getMonth()===m;});res.push({label:mn[m],completed:ms.filter(s=>s.status==="completed").length,failed:ms.filter(s=>s.status==="failed").length,total:ms.length});}}return res;}
  ...
  const bars=buildBars(period),maxBar=Math.max(...bars.map(b=>b.total),1);
  ...
  return(
    <div className="screen">
      ...
      <div className="progress-compare-row">
        <div className="compare-card"><div className="compare-label">Completion</div><div className="compare-val" style={{color:currPct>=70?"var(--success)":currPct>=40?"var(--accent)":"var(--danger)"}}>{currPct}%</div><div className="compare-change"><TrendArrow diff={pctDiff}/> <span style={{fontSize:11,color:"var(--text3)"}}>{periodLabel[period]}</span></div></div>
        <div className="compare-card"><div className="compare-label">Focus Time</div><div className="focus-time-big" style={{fontSize:24}}>{currFocus}m</div><div className="compare-change"><TrendArrow diff={focusDiff}/> <span style={{fontSize:11,color:"var(--text3)"}}>minutes</span></div></div>
      </div>
      <div className="progress-compare-row">
        <div className="compare-card"><div className="compare-label">Sessions</div><div className="compare-val">{curr.length}</div><div className="compare-change"><TrendArrow diff={curr.length-prev.length}/> <span style={{fontSize:11,color:"var(--text3)"}}>{periodLabel[period]}</span></div></div>
        <div className="compare-card"><div className="compare-label">Avg Length</div><div className="focus-time-big" style={{fontSize:24,color:"var(--blue)"}}>{currAvg}m</div><div className="text-xs mt-4">per session</div></div>
      </div>
      <div className="label mb-12">SESSIONS OVER TIME</div>
      <div className="card mb-20"><div className="chart-wrap"><div className="bar-chart-vert">{bars.map((b,i)=>(<div key={i} className="vert-bar-group"><div className="vert-bar-val">{b.total>0?b.total:""}</div><div className="vert-bar-track" style={{height:100}}>{b.failed>0&&<div className="vert-bar-fill" style={{height:`${(b.failed/maxBar)*100}%`,background:"var(--danger)",opacity:0.6}}/>}{b.completed>0&&<div className="vert-bar-fill" style={{height:`${(b.completed/maxBar)*100}%`,background:"var(--success)"}}/>}</div><div className="vert-bar-label">{b.label}</div></div>))}</div></div><div style={{display:"flex",gap:16,marginTop:12,justifyContent:"center"}}><div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"var(--text3)"}}><div style={{width:10,height:10,borderRadius:2,background:"var(--success)"}}/>Completed</div><div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"var(--text3)"}}><div style={{width:10,height:10,borderRadius:2,background:"var(--danger)",opacity:0.6}}/>Failed</div></div></div>
      <div className="label mb-12">28-DAY ACTIVITY</div>
      <div className="card mb-20"><div className="heatmap-day-labels">{["M","T","W","T","F","S","S"].map((d,i)=><div key={i} className="heatmap-day-label">{d}</div>)}</div><div className="heatmap-wrap">{heatmap.map((cell,i)=>(<div key={i} className={`heat-cell heat-${Math.min(cell.count,4)}`}/>))}</div><div style={{display:"flex",gap:6,alignItems:"center",justifyContent:"flex-end",marginTop:4}}><span style={{fontSize:10,color:"var(--text3)"}}>Less</span>{[0,1,2,3,4].map(n=><div key={n} className={`heat-cell heat-${n}`} style={{width:10,height:10,flexShrink:0,borderRadius:2}}/>)}<span style={{fontSize:10,color:"var(--text3)"}}>More</span></div></div>
...
export default function App(){
  // ── Bootstrap state ──
  const [appReady,setAppReady]=useState(false);       // false while checking Supabase session
  const [user,setUser]=useState(null);                // Supabase user object
  const [profile,setProfile]=useState(null);          // profiles row
  const [sessions,setSessions]=useState([]);          // sessions array from Supabase
  const [completedCount,setCompletedCount]=useState(0); // from Supabase, for free gate
  ...
  // ── Load profile + sessions for a user ────────────────────────────────────
  const loadUserData=useCallback(async(u)=>{
    // Fetch profile (trigger may have created it already; fallback if not)
    let prof=await fetchProfile(u.id);
    if(!prof){
      prof=await createProfile(u.id,u.email);
    }
    setProfile(prof);

    // If local onboarding was completed before sign-in, sync it up
    ...
    // Load sessions
    const rows=await fetchSessions(u.id);
    setSessions(rows);
  ...
  useEffect(()=>{
    ...
    const{data:{subscription}}=supabase.auth.onAuthStateChange(async(event,session)=>{
      if(session?.user){
        ...
        setUser(null);setProfile(null);setSessions([]);setCompletedCount(0);
      }
    });
    return()=>subscription.unsubscribe();
  },[loadUserData]);

  // ── Refresh sessions after a session is saved ─────────────────────────────
  ...
  // ── Onboarding done (before auth) ─────────────────────────────────────────
  const handleOnboardDone=()=>{
    save(SK.ONBOARD_LOCAL,true);
  ...
  // 1. Onboarding (before auth)
  if(!onboardingDone){
    return <><style>{CSS}</style><Onboarding onDone={handleOnboardDone}/></>;
  }

  // 2. Auth
  if(!user){
    return <><style>{CSS}</style><AuthScreen onAuth={handleAuth}/></>;
  }

  // 3. Paywall (full-screen)
  if(showPaywall){
    return(
      <><style>{CSS}</style>
  ...
  if(activeSession){
    return(
      <><style>{CSS}</style>
      <FocusSession session={activeSession} onComplete={handleSessionComplete} onFail={handleSessionFail}/>
      </>
    );
  }
  ...
  if(sessionOutcome){
    return(
      ...
      {sessionOutcome.type==="success"
        ?<CompletionScreen
            session={{...sessionDraft,notes:sessionOutcome.notes}}
            userId={user.id}
            stats={stats}
        ...
        :<div className="app">
          <FailureScreen
            session={{...sessionDraft,elapsed:sessionOutcome.elapsed,notes:sessionOutcome.notes}}
            userId={user.id}
            onRestart={()=>{setSessionOutcome(null);handleNewSession();}}
            onDashboard={handleOutcomeDone}
            onSessionSaved={refreshSessions}
          />
         </div>
      }
  ...
  if(showPlan){
    return(
      <><style>{CSS}</style>
      <div className="app"><PlanSession onSave={handlePlanSaved} onCancel={()=>setShowPlan(false)}/></div>
      </>
    );
  }

  // 7. Session setup flow
  if(setupStep){
    return(
      <><style>{CSS}</style>
      <div className="app">
  ...
  return(
    ...
    <div className="app">
      {tab==="home"&&<Dashboard
        ...
        onPlanSession={()=>setShowPlan(true)}
        onStartPlanned={handleStartPlanned}
        highlightPlanned={highlightPlanned}
